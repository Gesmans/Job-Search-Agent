...
def generate_cover_letter(job_description, cv_content, job_title):
        response = client.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=1024,
            system="You are a cover letter generator. Generate a personalized cover letter based on the job description and the CV content. Keep it concise, direct, and below 100 words.",
            messages=[{"role": "user", "content": f"Here is the job description: {job_description} Here is the CV content: {cv_content} Generate a cover letter that highlights the most relevant skills and experiences from the CV that match the job description."}]
        )
...
def is_job_active(job_description):
    response = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=10,
        messages=[{"role": "user", "content": f"Is this job still active and accepting applications? Reply only 'yes' or 'no': {job_description[:1000]}"}]
    )
    reply = ""
    for block in response.content:
        if block.type == "text":
